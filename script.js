...
const toggleBtns = [themeToggleBtn].filter(Boolean);
...
if (toggleBtns.length > 0) {
  ...
  if (currentTheme === 'light') {
    toggleBtns.forEach(btn => btn.innerHTML = moonIcon);
  }
  ...
  toggleBtns.forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (theme === 'light') {
        ...
        toggleBtns.forEach(b => b.innerHTML = sunIcon);
      } else {
        document.documentElement.setAttribute('data-theme', 'light');
        localStorage.setItem('theme', 'light');
        toggleBtns.forEach(b => b.innerHTML = moonIcon);
      }
    });
  });
}
